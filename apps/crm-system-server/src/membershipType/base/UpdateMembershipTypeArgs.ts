/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { MembershipTypeWhereUniqueInput } from "./MembershipTypeWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { MembershipTypeUpdateInput } from "./MembershipTypeUpdateInput";

@ArgsType()
class UpdateMembershipTypeArgs {
  @ApiProperty({
    required: true,
    type: () => MembershipTypeWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => MembershipTypeWhereUniqueInput)
  @Field(() => MembershipTypeWhereUniqueInput, { nullable: false })
  where!: MembershipTypeWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => MembershipTypeUpdateInput,
  })
  @ValidateNested()
  @Type(() => MembershipTypeUpdateInput)
  @Field(() => MembershipTypeUpdateInput, { nullable: false })
  data!: MembershipTypeUpdateInput;
}

export { UpdateMembershipTypeArgs as UpdateMembershipTypeArgs };
